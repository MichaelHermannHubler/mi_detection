...
class CPSC2018Dataset(Dataset):
    def __init__(self):
        super().__init__()
        self.path = "G:\\Projects\\MA\\cpsc2018\\data"
        self.data = loadCPSC2018()
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            # read the .mat file corresponding to the given index
            mat = scipy.io.loadmat(os.path.join(self.path, self.data['Recording'][idx] + ".mat"))
            # extract the signals and labels from the mat file
            signals = mat['ECG'][0][0][2][:,:5000]
...
if __name__=="__main__":
    ...
    mat = scipy.io.loadmat(os.path.join("G:\\Projects\\MA\\cpsc2018\\data", 'a0239' + ".mat"))
    signal, _, _, _ = dataset[948]
    # print(signal.size())
    ecg_plot.plot(signal, sample_rate=500, )
    ecg_plot.show()